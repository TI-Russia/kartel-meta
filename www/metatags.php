...
{ $res='';
  $sql='select m.filename,b.creator,b.modifier,b.company,a.contentid
	from meta_zakupki.dbo.datafiles as a  
          inner join meta_zakupki.dbo.metafiles as m on a.contentid=m.contentid
	   inner join meta_zakupki.dbo.LineTags as b on b.fileId=m.id where a.purchasenumber='."'".$purchase."'";
...
echo '<form method="get" action="/metatags.php">
...
' ИНН Исполнителя:<input name="iinn" value="'.$iinn.'" '.$inputstyle.'<br><br>
        ...
        <input type=submit value="Найти" formaction="/metatags.php" '.$submitstyle."\n".
